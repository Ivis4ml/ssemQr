...
Xin Zhou and Xiaodong Cai. Identification of trans-eQTLs via Joint eQTL mapping and inference of Gene Regultory Network Bioinformatics, submitted.
...
Y     = data$Data$Y
Sk    = data$Data$Sk
gamma = cv.ridgeRegression(X, Y, Sk, ngamma = 100, nfold = 10, data$Vars$n, data$Vars$p, data$Vars$q)
fit0   = ridgeRegression(X, Y, Sk, gamma, data$Vars$n, data$Vars$p, data$Vars$q, trans = FALSE, sparse = FALSE)
```
...
fitOpt = opt.SSEMiPALM(X = X, Y = Y, B = fit0$B, F = fit0$F, Sk = Sk, sigma2 = fit0$sigma2,
                       nlambda = 7, nrho = 7, p = data$Vars$p, wt = TRUE)
